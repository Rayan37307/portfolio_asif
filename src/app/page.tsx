  ...
  return (
    <main>
      ...
      <section style={{ minHeight:"100svh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"8rem 1.5rem 5rem", position:"relative", overflow:"hidden" }}>
        ...
        <Reveal delay={0.1}>
          <h1 className="heading" style={{ fontSize:"clamp(3.5rem, 12vw, 10rem)", maxWidth:"900px" }}>
            Muhammad<br />
        ...
        <Reveal delay={0.2}>
          <p style={{ marginTop:"2rem", maxWidth:"36rem", color:"var(--fg-muted)", fontSize:"1.05rem", lineHeight:1.75 }}>
      ...
      <section style={{ maxWidth:"1200px", margin:"0 auto", padding:"6rem 1.5rem", textAlign:"center" }}>
        ...
        <StaggerContainer style={{ display:"flex", flexWrap:"wrap", gap:"4rem", justifyContent:"center", alignItems:"center", opacity:0.6, filter:"grayscale(100%)" }}>
          {["FORBES", "TECHCRUNCH", "ENTREPRENEUR", "GQ", "BUSINESS INSIDER"].map((logo) => (
            <StaggerItem key={logo}>
              <span className="heading" style={{ fontSize:"1.5rem", letterSpacing:"0.1em", color:"var(--fg-muted)" }}>
                {logo}
              </span>
            </StaggerItem>
          ))}
      ...
      <section style={{ maxWidth:"1200px", margin:"0 auto", padding:"8rem 1.5rem" }}>
        <Reveal>
          <div className="card" style={{ padding:"4rem", display:"flex", flexDirection:"column", alignItems:"center", textAlign:"center", background:"radial-gradient(circle at center top, rgba(82,113,255,0.08) 0%, var(--bg-card) 60%)", borderTop:"2px solid var(--accent)" }}>
            ...
            <StaggerContainer style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))", gap:"2rem", width:"100%", maxWidth:"800px", marginBottom:"3.5rem", textAlign:"left" }}>
              {[
                ...
                <StaggerItem key={i}>
                  <div style={{ padding:"1.5rem", background:"var(--bg)", border:"1px solid var(--border)", borderRadius:"4px" }}>
      ...
      <section id="content" style={{ maxWidth:"1200px", margin:"0 auto", padding:"8rem 1.5rem" }}>
        ...
        <StaggerContainer style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))", gap:"1.25rem" }}>
          {videos.length > 0 ? videos.map(v => (
            <StaggerItem key={v.id}>
              <a href={v.link} target="_blank" rel="noopener noreferrer" className="vcard">
      ...
      <section style={{ maxWidth:"1200px", margin:"0 auto", padding:"8rem 1.5rem" }}>
        <Reveal>
          <div style={{ marginBottom:"3.5rem" }}>
            <p className="eyebrow" style={{ marginBottom:"1rem" }}>Must Watch</p>
            <h2 className="heading" style={{ fontSize:"clamp(2rem, 5vw, 3.5rem)", marginBottom:"0.75rem" }}>
              Banger <span style={{ color:"var(--accent)" }}>Contents.</span>
            </h2>
            <p style={{ color:"var(--fg-muted)", fontSize:"1rem", lineHeight:1.75, maxWidth:"520px" }}>
              Hand-picked videos that hit different. These are the ones that changed everything.
            </p>
          </div>
        </Reveal>

        <StaggerContainer style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(340px, 1fr))", gap:"1.5rem" }}>
          {[
            { id: "O7cYmFH6GwE", thumb: "/banger-1.avif", title: "Education Trap: যেভাবে 99.63% মানুষ ভুল পথে দৌড়াচ্ছে 🤫", views: "Viral" },
          ].map((video, i) => (
            <StaggerItem key={i}>
              <a href={`https://youtu.be/${video.id}`} target="_blank" rel="noopener noreferrer" className="vcard">
                <div style={{ position:"relative", aspectRatio:"16/9", overflow:"hidden", background:"#111" }}>
                  <Image src={video.thumb} alt={video.title} fill className="vthumb" style={{ objectFit:"cover" }} />
                  <div aria-hidden style={{ position:"absolute", inset:0, background:"linear-gradient(to top, rgba(0,0,0,0.7) 0%, transparent 60%)", zIndex:1 }} />
                  <div className="vplay">
                    <div style={{ width:"3.5rem", height:"3.5rem", borderRadius:"50%", background:"red", display:"flex", alignItems:"center", justifyContent:"center" }}>
                      <Play style={{ width:"1.2rem", height:"1.2rem", color:"#fff", marginLeft:"2px" }} />
                    </div>
                  </div>
                </div>
                <div style={{ padding:"1.25rem" }}>
                  ...
                  <h3 className="vtitle heading" style={{ fontSize:"1.25rem", lineHeight:1.1, color:"var(--fg)" }}>
                    {video.title}
                  </h3>
                </div>
              </a>
